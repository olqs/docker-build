...
app.config['MYSQL_DATABASE_HOST'] = os.environ.get("MYSQL_DATABASE_HOST", default='mariadb')
...
def hello():
  try:
    ...
    for row in rows:
      subdomains = ["main", "imap", "smtp", "pop3", "autodiscover", "autoconfig", "webmail", "email"]
      domains = [ domain + "." + row[0] for domain in subdomains]
      tls_domains += [ { 'main' : "mail." + row[0], 'sans' : domains }]
    
    traefik_config = {'traefik' : { 'http' : { 'routers' : { TRAEFIK_ROUTER : { 'tls' : { 'domains' : tls_domains } }}}}}
